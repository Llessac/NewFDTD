# NewFDTD v0.2-20260629
-修复许多BUG

-增加Cupy并行，示例代码运行时间从84.65s降低到12.52s

-增加mpi并行，添加了test_mpi示例代码，能够使用多核CPU运行甚至多显卡运行
...
开启GPU加速，请保证安装了CUDA
使用pip install cupy-cuda1?x[ctk] ?中为GPU版本，使用nvidia-smi命令查看
>import os
>os.environ['FDTD_GPU'] = '1'   
...
>python test.py
并行MPI使用命令(示例为4核)
>mpiexec -np 4 python test_mpi.py
或
